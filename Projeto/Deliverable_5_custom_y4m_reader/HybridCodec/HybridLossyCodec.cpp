...
public:
    ...
    //  WHAT TO TODO -----------------------------------------------
    //  É só alterar aqui
    //  encoder needs to keep track of what the decoder will "think" when he is decoding
    //  if the difference in values is too small for a short period of time (1 frame), encode 0 difference
    //  if the difference is big, encode the value as normal
    //  if the difference between some frames start adding up, encode the difference from what the decoder "predicts"
    // to the actual value.

    //  Example: one pixel's value over 5 frames

    //  Actual Value 100  -  101  -  102  -  103  -  104
    //  Actual Diff    0  -    1  -    1  -    1  -    1
    //  Encoded Diff   0  -    0  -    2  -    0  -    2
    //  What the
    //decod predict 0->   100->    100->   102->   102->   
    //  Encoded Val  100  -  100  -  102  -  102  -  104
    
    //  Loss of data, but more 0s encoded, (less bits)
